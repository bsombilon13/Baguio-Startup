...
const EventModal: React.FC<EventModalProps> = ({ event, onClose }) => {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/20 dark:bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div 
      ...
      >
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 bg-white/50 dark:bg-black/50 hover:bg-white dark:hover:bg-slate-800 text-slate-900 dark:text-white p-2 rounded-full transition-colors z-10 shadow-sm backdrop-blur-md"
        >
          <X size={20} />
        </button>
        ...
        <div className="p-6 relative">
          ...
          <button className="w-full bg-slate-900 dark:bg-white hover:bg-slate-800 dark:hover:bg-slate-200 text-white dark:text-slate-900 font-bold py-3 px-6 rounded-xl transition-all shadow-lg hover:shadow-xl">
            Register for Event
          </button>
        </div>
      </div>
