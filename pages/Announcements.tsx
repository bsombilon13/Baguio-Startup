  ...
  return (
    <div className="space-y-6 pb-24 h-full flex flex-col">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button className="bg-[#35308f] hover:bg-[#2a2670] text-white px-6 py-2.5 rounded-xl font-bold shadow-lg shadow-indigo-200 dark:shadow-none transition-all flex items-center gap-2 w-fit">
          <Megaphone size={18} />
          Submit
        </button>
      </div>

      <div className="flex gap-2 border-b border-slate-200 dark:border-slate-800 pb-1 overflow-x-auto no-scrollbar">
        {tabs.map((tab) => (
          <button
            key={tab.name}
            onClick={() => setActiveTab(tab.name)}
            className={`
              flex items-center gap-2 px-4 py-2.5 rounded-t-lg font-bold text-sm whitespace-nowrap transition-all border-b-2
              ${activeTab === tab.name 
