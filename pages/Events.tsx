    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">Workshops, meetups, and conferences in the region.</p>
        </div>
        ...
        <div className="bg-white p-1 rounded-xl flex gap-1 border border-slate-200 shadow-sm">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${
              view === 'list' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${
              view === 'calendar' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'
            }`}
          >
          ...
          {events.map((event, i) => (
            ...
            >
              <div className="mt-auto">
                 <div className="bg-white/90 w-fit px-3 py-1 rounded-full text-xs font-bold text-slate-900 mb-2 backdrop-blur-md shadow-sm">
      ...
      {view === 'calendar' && (
        <div className="bg-white border border-slate-200 shadow-sm rounded-[2rem] p-6 md:p-8">
          ...
          <div className="grid grid-cols-7 gap-2 md:gap-4">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
              <div key={day} className="text-center text-slate-400 text-xs font-bold uppercase tracking-wider py-2">
                {day}
              </div>
            ))}
              ...
              return (
                <div 
                  ...
                  className={`
                    ...
                    ${isTodayDate ? 'border-indigo-500 bg-indigo-50 shadow-inner' : 'border-slate-100 bg-slate-50 hover:bg-white hover:shadow-md'}
                    ${dayEvents.length > 0 ? 'cursor-pointer hover:border-indigo-300' : ''}
                  `}
                  onClick={() => dayEvents.length > 0 && setSelectedEvent(dayEvents[0])}
                >
                  <span className={`text-sm font-bold ${isTodayDate ? 'text-indigo-600' : 'text-slate-500'}`}>
                    {format(day, 'd')}
                  </span>
                  ...
                  {dayEvents.length > 0 && (
                    <div className="mt-1 md:mt-2 space-y-1">
                      {dayEvents.map(e => (
                        <div key={e.id} className="w-full h-1.5 md:h-2 bg-gradient-to-r from-pink-400 to-indigo-500 rounded-full" />
                      ))}
                      {/* Tooltip for desktop */}
